...
def sync_gradients(model, rank, world_size):
    ...
    for param in model.parameters():
        ...
        if rank == 0:
            gradients = [torch.zeros_like(param.grad.data) for _ in range(world_size)]
            
            # Gather and scatter
            dist.gather(param.grad.data, gradients, dst=0)
            mean = torch.mean(torch.stack(gradients), dim=0)
            gradients = [mean for _ in range(world_size)]
            dist.scatter(param.grad.data, gradients, src=0)
